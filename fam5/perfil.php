	...
	<script>
		...
		$(document).ready(function() {
			...
			function listarCarrinho() {
				...
				$.ajax({
					url: 'listar_carrinho.php', // Arquivo PHP que faz a consulta e retorna os pedidos
					type: 'GET',
					data: {
			...
			$('#sendOrderButton').on('click', function() {
				...
				if (produtosSelecionados.length > 0) {
					...
					$.ajax({
						url: 'enviar_pedido_carrinho.php', // Arquivo PHP que processa o pedido
						type: 'POST',
						data: {
        ...
        `);
			...
			$('#removeSelectedButton').on('click', function() {
				...
				if (produtosSelecionados.length > 0) {
					...
					$.ajax({
						url: 'remover_carrinho.php', // Arquivo PHP que processa a remoção
						type: 'POST',
						data: {
							produtos: produtosSelecionados
